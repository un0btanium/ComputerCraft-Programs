...
local version = "Planetz Under Gravity 0.3.0"
...
function checkForCollision(p,x,y)
...
function escMenu()
 ...
 if event[4] >= 1 and event[4] <= 10 then
  return "continue"
 elseif event[4] >= 11 and event[4] <= 14 then
  return "restart"
 elseif event[4] >= 15 and event[4] <= h then
  return "exit"
 end
end
...
function play()
 ...
 while amountPlanets > warpedPlanets do
  ...
  elseif event[1] == "monitor_touch" then
   if event[4] >= 1 and event[4] <= 7 then
    direction = "up"
   elseif event[3] >= 30 and event[3] <= w+1 then
    direction = "right"
   elseif event[4] >= 18 and event[4] <= h+1 then
    direction = "down"
   elseif event[3] >= 1 and event[3] <= 10 then
    direction = "left"
   else
 ...
 if amountPlanets == warpedPlanets then
  clear()
  setFancyOr(colors.white)
  centered(playingLevelName,7)
  centered("Gravity Modifications:",10)
  centered(tostring(directionChanges), 12)
  centered("Time:",15)
  centered(tostring(neededTime), 17)
  local event = { os.pullEvent("monitor_touch") }
 ...
 if editorGUI then
  if yPos == 1 then
   ...
   elseif xPos == 22 or xPos == 23 then
    selectedSpace = "O"
   elseif xPos >= 26 and xPos <= 29 then
    extendedGUI = not extendedGUI
   elseif xPos >= 31 and xPos <= 34 then
    return "save"
   elseif xPos >= 36 then
    return "exit"
   end
  ...
  elseif extendedGUI and yPos == 2 then
   ...
   elseif xPos == 28 or xPos == 29 then
    selectedSpace = "8"
   elseif xPos == 31 or xPos == 32 then
    selectedSpace = "M"
   end
   return "true"
  end
...
function pastebinImport()
 ...
 shell.run("pastebin","get",accualLink,"PUG_" .. levelnumber)
...
function saveLevel()
 ...
 local file = fs.open("PUG_"..levelnumber , "w")
...
function loadLevel(levelName)
 ...
 local moonnumber = 1
 local file = fs.open("PUG_"..levelName,"r")
...
function selectLevel(editorMode, header)
 local level = {}
 for i=1,1000 do
  if fs.exists("PUG_" .. i) then
   local file = fs.open("PUG_"..i ,"r")
 ...
 while true do
  clear()
  setFancyOr(colors.white)
  centered(header .. ": Level " .. more+1 .. " to " .. more+15 , 1)
  ...
  if actions-more > 15 then
   centered("More level",23)
  elseif more > 0 then
   centered("Back to the first level",23)
  end
  if editorMode then
   centered("Create a new level",24)
  end
  centered("Back to the menu",25)
  ...
  elseif event[4] == 23 then
   if actions-more > 15 then
    more = more + 15
   elseif more > 0 then
    more = 0
   end
  elseif editorMode and event[4] == 24 then
   resetPlayfield()
   return true
  elseif event[4] == 25 then
   return false
  end  
 end
end
...
function main()
 ...
 centered("DELETE",15)
 centered("QUIT",18)
 centered("by UNOBTANIUM",26)
 term.setTextColor(colors.white)
 local event = { os.pullEvent("monitor_touch") }
 if event[4] >= 6 and event[4] <= 7 then
  if selectLevel(false, "Play") then
   again = play()
  end
 elseif event[4] >= 9 and event[4] <= 10 then
  if selectLevel(true, "Editor") then
   levelEditor()
  end
 elseif event[4] >= 12 and event[4] <= 13 then
  pastebinImport()
 elseif event[4] >= 15 and event[4] <= 16 then
  if selectLevel(false, "Delete") then
   deleteLevel()
  end
 elseif event[4] >= 18 and event[4] <= 19 then
  clear()
  return false
 elseif event[4] == 26 then
  if fancy then
